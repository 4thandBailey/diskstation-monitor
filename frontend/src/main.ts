...
if (!token) window.location.href = '/dsm-auth.html';
...
(window as Window & { showPage: typeof showPage }).showPage = showPage;
...
(window as Window & { handleLogout: typeof handleLogout }).handleLogout = handleLogout;

// ── Types ─────────────────────────────────────────────────────────────────────
interface PollResult {
  outcome: string; latency_ms: number; ip_observed: string;
  polled_at: string; cpu_percent?: number; ram_percent?: number;
  connections?: { total?: number };
}
interface Device {
  id: string; serial: string; name: string; model: string;
  ip_address: string; port: number; protocol: string; status: string;
  dsm_version: string | null; latitude: number | null; longitude: number | null;
  location_name: string | null; last_poll: PollResult | null;
}
interface Alert {
...
function renderFleetTable(devs: Device[]) {
  ...
  tbody.innerHTML = devs.map(d => {
    ...
    const p  = d.last_poll;
    const tl = Array(20).fill('ok').map((o, i) => i === 8 ? 'slow' : o)
      .map(o => `<div class="poll-tick ${o}"></div>`).join('');
    const conns = p?.connections?.total ?? '—';
    return `<tr>
      <td><span class="status-dot ${sc}"></span>${capitalize(d.status)}</td>
...
function appendPollLog(devs: Device[]) {
  ...
  const entries = devs.filter(d => d.last_poll).slice(0, 10).map(d => {
    ...
    return `<div class="poll-log-entry">
      <span class="log-time">${t}</span><span class="log-device">${d.name}</span>
  ...
  const set = (id: string, val: string | number) => { const el = document.getElementById(id); if (el) el.textContent = String(val); };
  set('pe-healthy', online.length); set('pe-latency', avgLat || '—');
  set('pe-rate', `${rate}%`);       set('pe-last', new Date().toLocaleTimeString());
}
...
async function loadAlerts() {
  ...
  try {
    ...
    const cAlerts    = document.getElementById('c-alerts');
    ...
    if (cAlerts)    cAlerts.textContent    = String(alerts.length);
...
function ackAlert(id: string) { api(`/api/alerts/${id}/acknowledge`, { method: 'PATCH' }).then(() => loadAlerts()).catch(() => {}); }
function dismissAlert(id: string) { api(`/api/alerts/${id}`, { method: 'DELETE' }).then(() => { document.getElementById(`alert-${id}`)?.remove(); }).catch(() => {}); }
function dismissAllAlerts() { api('/api/alerts', { method: 'DELETE' }).then(() => loadAlerts()).catch(() => {}); }

(window as Window & { ackAlert: typeof ackAlert }).ackAlert = ackAlert;
(window as Window & { dismissAlert: typeof dismissAlert }).dismissAlert = dismissAlert;
(window as Window & { dismissAllAlerts: typeof dismissAllAlerts }).dismissAllAlerts = dismissAllAlerts;
...
function revokeToken(deviceId: string) {
  if (!confirm('Revoke this token? The device will stop polling until a new token is registered.')) return;
  api(`/api/devices/${deviceId}/token`, { method: 'DELETE' }).then(() => loadSecurity()).catch(() => {});
}
(window as Window & { revokeToken: typeof revokeToken }).revokeToken = revokeToken;

// ── Add Device ────────────────────────────────────────────────────────────────
function openAddDevice()  { document.getElementById('addDeviceModal')?.classList.add('open'); }
function closeAddDevice() { document.getElementById('addDeviceModal')?.classList.remove('open'); }
...
function viewDevice(_id: string) { showPage('integrityPage'); }

async function removeDevice(id: string) {
  if (!confirm('Remove this device? All poll history will be deleted.')) return;
  try { await api(`/api/devices/${id}`, { method: 'DELETE' }); loadFleet(); } catch { /* ok */ }
}

(window as Window & { openAddDevice: typeof openAddDevice }).openAddDevice = openAddDevice;
(window as Window & { closeAddDevice: typeof closeAddDevice }).closeAddDevice = closeAddDevice;
(window as Window & { submitAddDevice: typeof submitAddDevice }).submitAddDevice = submitAddDevice;
(window as Window & { viewDevice: typeof viewDevice }).viewDevice = viewDevice;
(window as Window & { removeDevice: typeof removeDevice }).removeDevice = removeDevice;

// ── Settings ──────────────────────────────────────────────────────────────────
async function saveSmtp() {
  const g = (id: string) => (document.getElementById(id) as HTMLInputElement | null)?.value.trim() ?? '';
  const host = g('smtpHost'), port = Number(g('smtpPort')) || 587;
  const username = g('smtpUser'), password = g('smtpPass'), fromName = g('smtpFromName');
...
async function deleteAccount() {
  const confirmed = prompt('Type DELETE to permanently erase your account and all data:');
...
(window as Window & { saveSmtp: typeof saveSmtp }).saveSmtp = saveSmtp;
(window as Window & { downloadData: typeof downloadData }).downloadData = downloadData;
(window as Window & { deleteAccount: typeof deleteAccount }).deleteAccount = deleteAccount;
